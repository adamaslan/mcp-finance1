...
Works both locally and on GCloud with Firebase integration.
"""
...
class IndustryService:
    ...
    Features:
    - Local development: Works with yfinance, no Firebase needed
    - Cloud deployment: Full Firebase caching and persistent storage
    - Multi-source APIs: Fallback through Finnhub → Alpha Vantage → yfinance
    - Smart caching: L1 (Firestore cache) → L2 (Persistent store)
    """
    ...
    def __init__(
        ...
        Args:
            alpha_vantage_key: Alpha Vantage API key (optional, for cloud).
            gcp_project_id: GCP project ID for Firebase (optional, enables cloud features).
            finnhub_key: Finnhub API key (optional, preferred data source).
        ...
        try:
            ...
            # Initialize persistent store for zero-API-cost reads (optional)
            try:
                ...
                logger.info("ℹ Persistent store unavailable (local mode): %s", e)
                self._persistent = None

            # Initialize data fetcher with fallback chain
            ...
            # Initialize Firebase cache (optional)
            try:
                self._cache = FirebaseCache(project_id=gcp_project_id)
                logger.info("✓ Firebase cache initialized")
            except FirebaseCacheError as e:
                logger.info("ℹ Firebase cache unavailable (local mode): %s", e)
                self._cache = None

            self._summary = SummaryGenerator()

            logger.info(
                "IndustryService initialized (mode=%s, finnhub=%s, persistent=%s, cache=%s)",
                "cloud" if gcp_project_id else "local",
                bool(finnhub_key),
    ...
    async def get_industry_performance(
        ...
        if use_cache and self._cache:
            try:
                ...
                if cached:
                    logger.info("✓ Returned cached data for %s", industry)
                    return cached
            except FirebaseCacheError as e:
    ...
    async def refresh_all_industries(
        ...
        if self._cache:
            try:
                ...
                logger.info(
                    "Batch cache check: %d cached, %d stale",
                    len(cached_data), len(stale),
                )
            except Exception as e:
    ...
    async def get_morning_summary(
        ...
        if not self._cache:
            raise IndustryServiceError(
                "Firebase cache not available. Cannot generate morning summary in local mode. "
                "Run POST /refresh-all with use_cache=False first."
            )
    ...
    async def _compute_industry_performance(
        ...
        """
        try:
            # Fetch full historical data (tries persistent → APIs → yfinance)
    ...
    async def get_top_performers(
        ...
        """
        if not self._cache:
            raise IndustryServiceError("Firebase cache required for this operation")
