...
class PersistentETFStore:
    ...
    def append_daily(
        ...
        """Append only new trading days to existing history.

        Reads the last stored date, then appends rows newer than that.
        This is the daily update path — costs 1 Firestore read + 1 write.

        Args:
            symbol: ETF ticker symbol.
            df: DataFrame with new data (may overlap with stored data).
            source: Data source for provenance.

        Returns:
            Number of new records appended (0 if already up-to-date).

        Raises:
            PersistentStoreError: If append fails.
        """
        symbol = symbol.upper()
        ...
        try:
            ...
            new_records = self._df_to_records(df)

            if metadata is None:
                # No existing data — store everything
                return self.store_full_history(symbol, df, source)

            last_stored = metadata.get("last_date", "")
            delta = [r for r in new_records if r["date"] > last_stored]

            if not delta:
                logger.debug("No new data for %s (last stored: %s)", symbol, last_stored)
                return 0

            doc_ref = self._db.collection(COLLECTION_NAME).document(symbol)
            storage_mode = metadata.get("storage_mode", "embedded")

            if storage_mode == "embedded":
                # Append to existing embedded array
                from google.cloud.firestore_v1 import ArrayUnion

                doc_ref.update({
                    "prices": ArrayUnion(delta),
                    "last_date": max(r["date"] for r in delta),
                    "last_updated": datetime.now(tz=timezone.utc).isoformat(),
                    "total_days": metadata.get("total_days", 0) + len(delta),
                    "source": source,
                })
            else:
                # Chunked mode: append to the correct year documents
                yearly = self._chunk_by_year(delta)
                for year_str, year_records in yearly.items():
                    year_ref = doc_ref.collection("years").document(year_str)
                    ...
                    if year_doc.exists:
                        from google.cloud.firestore_v1 import ArrayUnion
                        year_ref.update({"prices": ArrayUnion(year_records)})
                    else:
                        year_ref.set({
                            "year": year_str,
                            "count": len(year_records),
            ...
            logger.info("Appended %d new days for %s", len(delta), symbol)
            return len(delta)
            ...
            raise PersistentStoreError(
                f"Failed to append data for {symbol}: {e}"
            ) from e
